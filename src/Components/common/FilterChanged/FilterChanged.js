...
export function FilterChanged() {
  ...
  document.title = "Замена фильтрующих элементов";
  ...
  const loadDataPage = async () => {
    const responseAllDates = await httpService.sendRequest(
      API_CONFIG.getFilterChangedEvents(installationId).PATH
    );
  ...
  const addDataFilter = async () => {
    ...
    const { PATH, METHOD } = API_CONFIG.addFilterChangedEvent(installationId);
      ...
      {isLoading ? (
        ...
        <div className="container">
          ...
          <form className="section-title">
            <fieldset>
              <legend>Замена фильтрующих элементов</legend>
