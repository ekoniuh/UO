...
export function WorkTypeChanged() {
  ...
  const loadDataPage = async () => {
    const responseAllDates = await httpService.sendRequest(
      API_CONFIG.getWorkTypeChangedEvents(installationId).PATH
    );
  ...
  const addDataWorkType = async () => {
    ...
    const { PATH, METHOD } = API_CONFIG.addWorkTypeChanged(installationId);
  ...
  const selectAction = () => {
    switch (typeModal) {
      case "ADD":
        return addDataWorkType();
      case "SAVE":
        return saveDataWorkType();
